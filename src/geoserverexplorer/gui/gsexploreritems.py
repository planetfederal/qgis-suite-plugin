...
class GsLayersItem(GsTreeItem):
    ...
    def acceptDroppedItem(self, tree, explorer, item):
        return []
        if isinstance(item, PgTableItem):
...
class GsCatalogItem(GsTreeItem):
    ...
    def _checkLayers(self):
        ...
        if len(layers) == 0:
            QtGui.QMessageBox.warning(config.iface.mainWindow(), 'Cannot publish',
                "No suitable layers can be found in your current QGIS project.\n"
                "You must open the layers in QGIS before trying to publish them.",
