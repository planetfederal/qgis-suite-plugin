...
from geoserverexplorer.test.utils import GROUP, WORKSPACE, WORKSPACEB, cleanCatalog, PT2,\
    PT1, PUBLIC_SCHEMA, getPostgresConnection, getGeoServerCatalog
from geoserverexplorer.gui.dialogs.layerdialog import PublishLayerDialog,\
    PublishLayersDialog
from geoserver.catalog import Catalog
from geoserverexplorer.qgis import layers
from geoserverexplorer.gui.dialogs.importvector import ImportIntoPostGISDialog
from geoserverexplorer.postgis.schema import Schema
...
class CreateCatalogDialogTests(unittest.TestCase):
    ...
    explorer = GeoServerExplorer(singletab = True)
    ...
    def testCreateCatalogDialogUsingExistingName(self):
        ...
        settings.remove("")
        settings.endGroup()
        settings.beginGroup("/GeoServer/GeoServer/name_2") 
        settings.remove("")
        settings.endGroup()
    ...
    def testLastCatalogNameIsShownByDefault(self):        
        ...
        settings = QSettings()
        settings.endGroup()
        settings.beginGroup("/GeoServer/GeoServer/catalogname") 
        settings.remove("")
        settings.endGroup()
...
class LayerDialogTests(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.explorer = GeoServerExplorer(singletab = True)
