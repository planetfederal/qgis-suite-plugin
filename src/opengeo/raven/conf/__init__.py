...
# TODO (vng): this seems weirdly located in opengeo.raven.conf.  Seems like
# it's really a part of opengeo.raven.transport.TransportRegistry
...
def load(dsn, scope=None, transport_registry=None):
    ...
    >>> opengeo.raven.load(dsn, locals())

    >>> # Return DSN configuration
    >>> options = opengeo.raven.load(dsn)
    """

    if not transport_registry:
        from opengeo.raven.transport import TransportRegistry, default_transports
...
def setup_logging(handler, exclude=EXCLUDE_LOGGER_DEFAULTS):
    ...
    >>> from opengeo.raven.handlers.logging import SentryHandler
    >>> client = Sentry(...)
    >>> setup_logging(SentryHandler(client))

    Within Django:

    >>> from opengeo.raven.contrib.django.handlers import SentryHandler
